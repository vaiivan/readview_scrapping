...
def main():
    with sync_playwright() as p:
        ...
        time.sleep(random.random()*5 + 2)
        # fiction = page.locator("#topic_F>.tagHighest",has_text="Fiction").first()
        # fiction.click()
        page.goto("https://www-booksinprint-com.ezproxy.hkpl.gov.hk/Search/Results?q={{(genre:[Action]%20OR%20subgenre:[Action])}}%20AND%20synd_profile_rec:[$ANY_VALUE$]")
        page.wait_for_load_state('domcontentloaded')
        print("Landed on Action Genre")
        page.goto("https://www-booksinprint-com.ezproxy.hkpl.gov.hk/Search/Results?q=&qs=1&&op=4&ps=100")
        page.wait_for_load_state('domcontentloaded')
        print("100 items per page")
        # page.select_option("#ddlRecsPerPage","100")
        # time.sleep(random.random()*5 + 2)
        # update = page.locator(".MarginTop10 .btn-default",has_text="Update")
        # update.click()
        time.sleep(random.random()*5 + 2)
        ...
        #     try:
        #         book['book_picture'] = page.evaluate("document.querySelectorAll('img.coverImage')[{}].src".format(i))
        #     except Exception as e:
        ...
        #     try:
        #         book['author'] = page.evaluate("document.querySelectorAll('.contentAlignment')[{}].childNodes[3].childNodes[0].childNodes[1].innerText".format(i))
        #     except Exception as e:
        #         print("Error on author", e)
        #     try:
        #         book['publisher'] = page.evaluate("document.querySelectorAll('.contentAlignment')[{}].childNodes[3].childNodes[2].childNodes[5].childNodes[1].innerText".format(i))
        #     except Exception as e:
        #         print("Error on publisher", e)
        #     try:
        #         book['publish_date'] = page.evaluate("document.querySelectorAll('.contentAlignment')[{}].childNodes[3].childNodes[2].childNodes[6].childNodes[1].innerText".format(i))
        #     except Exception as e:
        #         print("Error on publish_date", e)
        #     try:
        #         book['ISBN'] = page.evaluate("document.querySelectorAll('.contentAlignment')[{}].childNodes[3].childNodes[2].childNodes[1].childNodes[1].innerText".format(i))
        #     except Exception as e:
        #         print("ISBN", e)
        #     try:
...
#title
#document.querySelectorAll('.contentAlignment')[0].childNodes[1].innerText
#image
#document.querySelectorAll('img.coverImage')[0]
# author
# document.querySelectorAll('.contentAlignment')[0].childNodes[3].childNodes[0].childNodes[1].innerText
#ISBN
#document.querySelectorAll('.contentAlignment')[0].childNodes[3].childNodes[2].childNodes[1].childNodes[1].innerText
#Publisher
# document.querySelectorAll('.contentAlignment')[0].childNodes[3].childNodes[2].childNodes[5].childNodes[1].innerText
#Publish date
#document.querySelectorAll('.contentAlignment')[0].childNodes[3].childNodes[2].childNodes[6].childNodes[1].innerText
# https://www-booksinprint-com.ezproxy.hkpl.gov.hk/Search/Results?q=&qs=1&&op=4&ps=100
# https://www-booksinprint-com.ezproxy.hkpl.gov.hk/Search/Results?q=&qs=1&&op=3&pn=2